...
void ExecPID_PosLift(PID_Typedef *p, PosRamp *posRamp,PosCL_TypeDef *posCL){
	...
	p->errorF = posCL->encPoscurrentMoveDist_mm - posRamp->currentMoveTarget_mm;
	posCL->PID_positioningErr = p->errorF;
...
void ExecPID_PosLift_GBEncoder(PID_Typedef *p, PosRamp *posRamp,PosCL_TypeDef *posCL){
	float temp_Ki_term;
	float temp ;

	p->errorF = posCL->GBPoscurrentMoveDist_mm - posRamp->currentMoveTarget_mm;
	posCL->PID_positioningErr = p->errorF;
	// if error is -ve we are lagging, and we want to add PWM. so we invert the sign here
	// if error is positive we are faster than the target and we want to slow down. again invert the sign
	p->errorF = -1.0*p->errorF;
